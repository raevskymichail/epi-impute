...
pacman::p_load("readr", "rlist", "reshape2", "readxl", "feather")
...
tfs <- read_csv("scRNA_ATAC_task/INTREGNET_Reproducible/AnimalTFDB_TF.bed", col_names = FALSE)
tfs = tfs$X1
...
GSE96769_PeakFile_promoters_tfs = GSE96769_PeakFile_promoters[GSE96769_PeakFile_promoters$X13 %in% tfs,]
GSE96769_PeakFile_promoters_tfs = GSE96769_PeakFile_promoters_tfs[order(GSE96769_PeakFile_promoters_tfs$X13),]

GSE96769_scATACseq_matrix = merge(GSE96769_scATACseq_matrix, GSE96769_PeakFile_promoters_tfs[c('X13', 'PeakFile_Peak_ID')], by = 'PeakFile_Peak_ID')
...
GSE96769_TFs = GSE96769_scATACseq_matrix$X13
GSE96769_scATACseq_matrix$X13 = NULL

# Select only TFs intersected between ATAC and RNA
tfs_common = intersect(GSE96769_TFs, GSE117498_scRNAseq_dataset$gene)

GSE96769_scATACseq_matrix = GSE96769_scATACseq_matrix[GSE96769_TFs %in% tfs_common, ]
GSE96769_scATACseq_matrix = GSE96769_scATACseq_matrix[order(tfs_common),]

GSE117498_scRNAseq_dataset_TFs = GSE117498_scRNAseq_dataset[GSE117498_scRNAseq_dataset$gene %in% tfs_common, ]
GSE117498_scRNAseq_dataset_TFs = GSE117498_scRNAseq_dataset_TFs[order(tfs_common),]
...
# # Binirize the matrix
# GSE117498_scRNAseq_dataset_TFs_values = subset(GSE117498_scRNAseq_dataset_TFs, select = - gene)
# GSE117498_scRNAseq_dataset_TFs_values = as.data.frame(as.matrix((GSE117498_scRNAseq_dataset_TFs_values > 0) + 0))
# GSE117498_scRNAseq_dataset_TFs = cbind(GSE117498_scRNAseq_dataset_TFs$gene, GSE117498_scRNAseq_dataset_TFs_values)
# colnames(GSE117498_scRNAseq_dataset_TFs)[1] = "gene"

GSE117498_scRNAseq_dataset_TFs = dcast(melt(GSE117498_scRNAseq_dataset_TFs, id.vars = "gene"), variable ~ gene)
rownames(GSE117498_scRNAseq_dataset_TFs) = GSE117498_scRNAseq_dataset_TFs$variable
GSE117498_scRNAseq_dataset_TFs$cells = gsub('_.*', '', GSE117498_scRNAseq_dataset_TFs$variable)
GSE117498_scRNAseq_dataset_TFs = GSE117498_scRNAseq_dataset_TFs[order(GSE117498_scRNAseq_dataset_TFs$cells),]
GSE117498_scRNAseq_dataset_TFs$variable = NULL

sc_data_list <- split(GSE117498_scRNAseq_dataset_TFs , f = GSE117498_scRNAseq_dataset_TFs$cells)
